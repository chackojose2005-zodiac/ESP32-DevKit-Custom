...
The board features:
- **ESP32-WROOM-32D** module with integrated PCB antenna
...
| Board Layers         | 4 (Signal / Signal / Signal / Signal) |
| Board Dimensions     | 30.73 mm × 56.90 mm          |
...
3. **ESP32 Module** — ESP32-WROOM-32D with all GPIO broken out to headers
...
| MOD1                   | ESP32-WROOM-32D    | ESP32-WROOM32D.pretty:ESP32-WROOM-32D          |
...
| Layer 2    | In1.Cu       | Signal routing (inner 1)    |
| Layer 3    | In2.Cu       | Signal routing (inner 2)    |
| Layer 4    | B.Cu         | Signal routing (bottom)     |

All four copper layers are used for **signal routing**. Power (VCC/3.3 V) and ground (GND) connections are routed as **copper tracks** across all layers using KiCad's **autorouter**, rather than as dedicated power or ground planes. This approach was used to complete all 68 nets with 0 unrouted connections.

> **Note:** In a production design, dedicated inner ground and power planes (In1.Cu = GND, In2.Cu = PWR) are recommended for better RF performance and signal integrity. For this educational clone, autorouted tracks on all four layers achieve full connectivity.
...
ESP32-DevKit-Custom/
│
├── ESP32-DevKit-Custom.kicad_pro    # KiCad project file
├── ESP32-DevKit-Custom.kicad_sch    # Schematic
├── ESP32-DevKit-Custom.kicad_pcb    # PCB layout
│
├── ESP32-WROOM32D.pretty/           # Custom ESP32-WROOM-32D footprint
├── CP2102N-A01-GQFN28footprint.pretty/ # Custom CP2102N footprint
├── MMSS8050-H-TPfootprint.pretty/   # Custom MMSS8050 footprint
│
...
1. **Clone this repository:**
   ```bash
   git clone https://github.com/chackojose2005-zodiac/ESP32-DevKit-Custom.git
   cd ESP32-DevKit-Custom
   ```

2. **Open KiCad:**
   Launch KiCad and click **File → Open Project**, then select `ESP32-DevKit-Custom.kicad_pro`.
...
This design is faithful to the Espressif ESP32-DevKitC-v4 reference, with the following intentional modifications:

- **ESP32-WROOM-32D module used** — The design uses the ESP32-WROOM-32D variant with a custom footprint, featuring a single rectangular SMD thermal pad (pad 39) for GND instead of individual through-hole vias.

- **Autorouted PCB layout** — All nets (GND, 3.3 V, signals) are connected using KiCad's autorouter across all four copper layers (F.Cu, In1.Cu, In2.Cu, B.Cu). The result is **0 unrouted connections** across all 68 nets.

- **Voltage regulator (U2 / AMS1117-3.3) placed on the back copper layer (B.Cu)** — In the original Espressif reference, the regulator is on the front. Moving it to the back frees up front-side real estate for small passives, making hand-soldering easier.

- **Custom footprints** — The ESP32-WROOM-32D, CP2102N-A01-GQFN28, and MMSS8050-H-TP footprints were created from scratch and are included in the repository as local `.pretty` libraries.
